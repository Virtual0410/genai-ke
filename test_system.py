...
try:
    ...
    if response["message"]["content"]:
        print(f"{OK} Ollama responding: {response['message']['content'][:50]}...")
    else:
        print(f"{FAIL} Ollama returned empty response")
        sys.exit(1)
except Exception as e:
...
if data_path.exists():
    ...
    with open(data_path, 'r', encoding='utf-8') as f:
        chunks = json.load(f)
    print(f"{OK} Data loaded: {len(chunks)} chunks available")
else:
...
try:
    ...
    if result["answer"] and len(result["answer"]) > 20:
        ...
        print(f"{FAIL} Query returned empty or very short answer")
...
try:
    ...
    if any(phrase in answer_lower for phrase in ["not provide", "insufficient", "enough evidence"]):
        print(f"{OK} Correctly refused unsupported query")
        print(f"   Response: {result['answer'][:80]}...")
    else:
        print(f"{WARN} Warning: May have hallucinated answer to unsupported query")
        print(f"   Answer: {result['answer'][:100]}...")
except Exception as e:
    print(f"{FAIL} Refusal test failed: {e}")
...
try:
    ...
    if total_stances > 0:
        print(f"{OK} Stance detection working")
        ...
        print(f"{WARN} No stance information detected")
except Exception as e:
    print(f"{FAIL} Stance test failed: {e}")

print("\n" + "=" * 70)
print("TEST SUMMARY")
print("=" * 70)
print(f"{OK} System is operational")
print("")
print("Next steps:")
